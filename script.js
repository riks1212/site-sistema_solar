...
function selectPlanet(planet) {
  ...
  const elHoursDetail = elPlanet.querySelector('[data-detail="translação"]');
  animate.fromTo({
    from: +prevPlanet.translação,
    to: +currentPlanet.translação },
  value => {
    elHoursDetail.innerHTML = Math.round(value);
  });

  const elTiltDetail = elPlanet.querySelector('[data-detail="temperatura"]');
  animate.fromTo({
    from: +prevPlanet.temperatura,
    to: +currentPlanet.temperatura },
  value => {
    elTiltDetail.innerHTML = value.toFixed(1);
  });

  const elGravityDetail = elPlanet.querySelector('[data-detail="gravidade"]');
